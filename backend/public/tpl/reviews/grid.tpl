<div class="cms-content">
    ...
    <div class="panel panel-default panel-tabs" style="margin-bottom: 0px;" >
        <div class="panel-heading">
            <ul class="nav nav-tabs" role="tablist">
                <li class="active"><a href="#reviews/grid">Danh sách ý kiến</a></li>
            </ul>
        </div>
    </div>
    <!-- /panel -->
    ...
    <div class="func-container">
        <div class="func-yellow">
            <form method="GET" action="#reviews/grid" id="search" class="form row" >
                <div class="col-sm-3 padding-all-5">
                    <input data-search="name" name="name" type="text" class="form-control" placeholder="Tên ý kiến">
                ...
                <thead>
                    <tr class="success" >
                        ...
                        <th class="text-center" style="vertical-align: middle;width: 215px" > 
                            Chức năng<i style="cursor: pointer" onclick="reviews.add();" class="pull-right glyphicon glyphicon-plus">
                        </th>
                    <tr>
                </thead>
                <tbody>
                    <% if (data.length <= 0 ){ %>
                    <tr>
                        <td colspan="8" class="text-danger" style="text-align: center">Không tồn tại reivews nào trong cơ sở dữ liệu!</td>
                    </tr>
                    <% }else{ %>
                    <tr data-key=""></tr>
                    ...
                    <tr data-key="<%= this.id%>">
                        <td class="text-center" style="vertical-align: middle">
                            <p><%= eval(index+1) %></p>
                            <hr/>
                            <i class="glyphicon glyphicon-trash icon-remove" onclick="reviews.remove('<%= this.id %>');"></i>
                        </td>
                        <td class="text-center" style="vertical-align: middle">
                        ...
                        <td>
                            <div data-key="<%= this.id %>">
                                <%= '<label class="label label-' + (this.active == 1 ? 'success' : 'danger') + '" >' + (this.active == 1 ? 'Hoạt động' : 'Tạm khóa') + '</label><i onclick="reviews.changeActive(\'' + this.id + '\')" style="cursor: pointer" class="pull-right glyphicon glyphicon-' + (this.active == 1 ? 'check' : 'unchecked') + '" />' %>
                            </div>
                        </td>
                        <td>
                            <div data-key-home="<%= this.id %>">
                                <%= '<label class=" label label-' + (this.home == 1 ? 'success' : 'danger') + '" >' + (this.home == 1 ? 'Hiển thị' : 'Tạm khóa') + '</label><i onclick="reviews.changeHome(\'' + this.id + '\')" style="cursor: pointer" class="pull-right glyphicon glyphicon-' + (this.home == 1 ? 'check' : 'unchecked') + '" />' %>
                            </div>
                        </td>
                        ...
                        <td>
                            <div class="btn-group" style="margin-top: 5px">
                                <button onclick="reviews.edit('<%=this.id%>')" type="button" class="btn btn-success" style="width: 100px;"><span class="glyphicon glyphicon-edit pull-left" style="line-height: 18px"></span> Sửa</button>
                                <button onclick="image.addImage('<%= this.id %>', 'reviews');" type="button" class="btn btn-info" style="width: 100px;">
