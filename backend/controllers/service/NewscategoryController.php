...
class NewscategoryController extends ServiceController {
    ...
    /**
     * lấy cấu hình seo danh mục
     * @return type
     */
    public function actionGetmetacategory() {
        if (is_object($resp = $this->can("getmetacategory"))) {
            return $this->response($resp);
        }

        $id = Yii::$app->request->get('id');
        $metacategory = MetaCategoryBusiness::get($id);
        if (empty($metacategory)) {
            return $this->response(new Response(true, ''));
        }
        return $this->response(new Response(true, '', $metacategory));
    }

    /**
     * cấu hình seo danh mục
     * @return type
     */
    public function actionConfig() {
        ...
        $metacategory = new MetaCategoryForm();
        $metacategory->setAttributes(Yii::$app->request->getBodyParams());
        return $this->response($metacategory->save());
    }
